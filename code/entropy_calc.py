...
def calculate_entropy(config):
    ...
    for i in range(num_chunks):
        tasks.append( (chunk_size, seeds[i].generate_state(1)[0], 
                      num_digits_a, num_digits_b, operation, result_length) )
    if remainder:
        tasks.append( (remainder, seeds[-1].generate_state(1)[0],
                      num_digits_a, num_digits_b, operation, result_length) )

    # 并行执行任务
    results = thread_map(worker, tasks, max_workers=os.cpu_count(), 
...
def save_to_csv(entropies, config, result_length):
    ...
    with open(filename, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Position", "PlaceValue", "Entropy(bits)", 
                        "Operation", "NumDigitsA", "NumDigitsB"])
        for pos, e in enumerate(entropies):
            place_value = 10 ** (result_length - 1 - pos)
            writer.writerow([pos, place_value, round(e, 4), 
                            config["operation"], config["num_digits_a"], config["num_digits_b"]])

def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="Calculate entropy of digit positions in arithmetic operations.")
    parser.add_argument("--num_digits_a", type=int, default=3, help="Number of digits in the first operand")
    parser.add_argument("--num_digits_b", type=int, default=3, help="Number of digits in the second operand")
    parser.add_argument("--operation", type=str, choices=["multiply", "add"], default="multiply", help="Operation to perform: multiply or add")
    parser.add_argument("--num_samples", type=int, default=100000, help="Total number of samples to generate")
    parser.add_argument("--output_file", type=str, default="entropy_results.csv", help="Output CSV file name")
...
if __name__ == "__main__":
    ...
    # 打印摘要
    print("\nEntropy Summary:")
    print(f"Operation: {config['operation'].upper()} ({config['num_digits_a']}-digit and {config['num_digits_b']}-digit numbers)")
